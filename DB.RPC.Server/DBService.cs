...
public class DBService : IDbService
{
    ...
    {
        return Task.FromResult(_db.Schema.Tables[tableName].Rows.Select(x => x.Select(cell => cell.ToString() ?? "null")));
    }
    ...
    {
        var id = _db.Schema.Tables[tableName].AddRow(row.ToDictionary(x => x.ColumnName, x => (object?)x.Value.String ?? (object?)x.Value.Int ?? (object?)x.Value.Real));
        await _db.SaveAsync(_fileSystem);
        return id;
    }
